      ...
      <nav id="nav">
          ...
          <li v-for="(nav, index) in headerContents" :class="nav.class" :key="key('nav', index)">
            ...
            <ol v-if="nav.subs && nav.subs.length>0" class="subnav">
              <li v-for="(sub, index) in nav.subs" :key="key('subnav', index)">
...
const allNavs = [
  ...
  },
  {
    ...
    href: 'https://www.baidu.com/',
    subs: [
      {title:"重庆时时彩", href:'https://www.baidu.com/'},
      {title:"北京PK10", href:'https://www.baidu.com/'},
      {title:"泰国分分彩", href:'https://www.baidu.com/'}
    ],
  },
  {
    permission: [1, 2, 3, 10],
...
export default {
  ...
  watch: {
    permission: function (val, oldVal) {
      console.log("Permission change to:"+this.permission)
...
#nav{
  position:absolute;top:0;left:$logoWidth + 10;
  width: $navWidth;
  text-align:center;
}
#nav>ul{
...
#nav>ul>li{
  position:relative;
  float:left;
  width:80px;height:$headerContentHeight;line-height:$headerContentHeight;
}
